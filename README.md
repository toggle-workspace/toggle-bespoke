...
1. Invoke the `next build` script by running `npm run build` in your project root. This creates a `.next` directory with a production-ready admin bundle.
1. Finally run `npm run start` to run Node in production and serve Payload from the `.build` directory.
